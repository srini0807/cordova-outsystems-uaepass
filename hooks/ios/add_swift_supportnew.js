      ...
      glob('**/*Bridging-Header*.h', { cwd: pluginsPath }, (error, files) => {
        ...
        for (configName in buildConfigs) {
          if (!COMMENT_KEY.test(configName)) {
            ...
            // Do not override CocoaPods settings
            if (!xcodeProject.getBuildProperty('ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES', buildConfig.name)) {
            ...
            if (!xcodeProject.getBuildProperty('LD_RUNPATH_SEARCH_PATHS', buildConfig.name)) {
  ...
  const platformVersions = platformsOnFs.map(platform => {
    ...
    return new Promise((resolve, reject) => {
      childProcess.exec('"' + script + '"', {}, (error, stdout, stderr) => {
        ...
        resolve({ platform, version: stdout.trim() });
      });
    });
  });
